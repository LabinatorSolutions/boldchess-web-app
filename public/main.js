...
function setArrow(state) {
	_arrow = state;
	if (_arrow && _curmoves.length > 0 && _curmoves[0].eval != null)
		showArrow1(_curmoves[0].move);
	else showArrow1();
}
...
function updateInfo() {
	...
	if (
		_history[_historyindex].length > 3 &&
		_history[_historyindex][3] != null
	) {
	...
	for (let i = 0; i < _history.length; i++) {
		...
		const san =
			_history[i].length > 3 && _history[i][3] != null
				? _history[i][3]
				: "\u2605";
		const span2 = document.createElement("span");
...
function historyButtons() {
	document.getElementById("buttonBack").className =
		_historyindex > 0 ? "on" : "off";
	document.getElementById("buttonForward").className =
		_historyindex < _history.length - 1 ? "on" : "off";
}
...
function paintMouse(e, p) {
	...
	if (
		bounds(x1, y1) &&
		((_clickFromElem != null && _clickFromElem.className.indexOf(" h0") > 0) ||
			e.which === 3 ||
			e.button === 2)
	) {
		const pos = parseFEN(getCurFEN());
...
function addHistoryEval(index, score, depth, move) {
	if (
		_history[index].length < 2 ||
		_history[index][1] == null ||
		(_history[index][1] != null && _history[index][1].depth < depth)
	) {
		const black = _history[index][0].indexOf(" b ") > 0;
...
function evalNext() {
	for (let i = 0; i < _curmoves.length; i++) {
		if (_curmoves[i].depth < _analysisEngine.depth) {
			const curpos = _curmoves[i].fen;
			_analysisEngine.score = null;
			if (!_analysisEngine.waiting) return;
			_analysisEngine.waiting = false;
			const initialdepth = _analysisEngine.depth;
			let savedpv = [];
			_analysisEngine.eval(
				curpos,
				function done(str) {
	...
	for (let i = _history.length - 1; i >= 0; i--) {
		if (
			...
			_history[i][1] == null ||
			(_history[i][1] != null &&
				_history[i][1].depth < _analysisEngine.depth - 1)
		) {
			const curpos = _history[i][0];
			_analysisEngine.score = null;
			if (!_analysisEngine.waiting) return;
...
function highlightMove(index, state) {
	...
	if (_dragElement != null) return;
	const elem = document.getElementById("chessboard1");
	const x1 = _curmoves[index].move.from.x;
	const y1 = _curmoves[index].move.from.y;
	const x2 = _curmoves[index].move.to.x;
	const y2 = _curmoves[index].move.to.y;
...
function repaintStatic() {
	...
	requestAnimationFrame(() => {
		...
		const staticEvalListLast =
			_historyindex > 0
				? getStaticEvalList(parseFEN(_history[_historyindex - 1][0]))
				: null;
...
function showEvals() {
	...
	if (_curmoves.length > 0) {
		const sortfunc = (a, b) => {
			const a0 = a.eval == null ? -2000000 : a.eval * (_curmoves[0].w ? -1 : 1);
	...
	for (let i = 0; i < _curmoves.length; i++) {
		...
		if (_movesPv)
			...
			node6.appendChild(
				document.createTextNode((_curmoves[i].pvtext || "?").split(" ")[0]),
			);
		const node7 = document.createElement("SPAN");
		node7.className = "depth";
		node7.appendChild(document.createTextNode(_curmoves[i].depth | "?"));

		const text = getEvalText(_curmoves[i].eval, false);
		if (text.indexOf(".") >= 0) {
			const node4 = document.createElement("SPAN");
			node4.className = "numleft";
			node4.appendChild(
...
function showGraphTooltip(i, event) {
	if (
		...
		const pos = parseFEN(_history[i][0]);
...
function reloadMenu() {
	requestAnimationFrame(() => {
		...
		addMenuLine();
		addMenuItem("menuCoach", _coachModeLabel, "C", true, () => {
			toggleCoachMode();
			showHideMenu(false);
		});
		addMenuLine();
...
function getCircleClassName(i) {
	...
	if (_curmoves[i].eval != null && _curmoves[0].eval != null) {
		const etop = Math.max(-6, Math.min(6, _curmoves[0].eval / 100));
		const ecur = Math.max(-6, Math.min(6, _curmoves[i].eval / 100));
...
function getStaticEvalList(pos) {
	...
	if (_staticEvalListCache.length > _staticEvalListCacheSize)
		_staticEvalListCache.shift();
	return grouplist;
}
