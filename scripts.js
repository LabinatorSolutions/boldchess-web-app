...
data.push({"name":"Weak unopposed pawn","group":"Threats","text":"<b>$</b>. Check if our pawn is weak and unopposed. This penalty is applied only if opponent has rook or queen.","code":"function $$(pos, square) {\n  if (square == null) return sum(pos, $$);\n  if ($rook_count(colorflip(pos)) + $queen_count(colorflip(pos)) == 0) return 0;\n  if ($opposed(pos, square)) return 0;\n  var v = 0;\n  if ($isolated(pos, square)) v++;\n  if ($backward(pos, square)) v++;\n  return v;\n}","links":[],"eval":true,"squares":1,"highlight":2,"forwhite":true,"graph":true,"elo":{"value":"1.25","error":"4.5","link":"http://tests.stockfishchess.org/tests/view/5a74e8d40ebc5902971a9715"}});
...
data.push({"name":"King ring","group":"Helpers","text":"<b>$</b> is square occupied by king and 8 squares around king.","code":"function $$(pos, square) {\n  if (square == null) return sum(pos, $$);\n  for (var ix = -2; ix <= 2; ix++) {\n    for (var iy = -2; iy <= 1; iy++) {\n      if (board(pos, square.x + ix, square.y + iy) == \"k\"\n      && (iy >= -1 || square.y + iy == 0)\n      && (ix >= -1 && ix <= 1 || square.x + ix == 0 || square.x + ix == 7)) return 1;\n    }\n  }\n  return 0;\n}","links":[],"eval":true,"squares":1,"highlight":2,"forwhite":true,"graph":false});
data.push({"name":"Overload","group":"Threats","text":"<b>$</b>. [[Weak enemies]] which are not a pawn and are defended by opponent.\nThe idea is that these pieces must be defended. Their defenders have\ndramatically limited mobility, and they are vulnerable to our future\nattack.","code":"function $$(pos, square) {\n  if (square == null) return sum(pos, $$);\n  if (\"nbrq\".indexOf(board(pos, square.x, square.y)) < 0) return 0;\n  if (!$weak_enemies(pos, square)) return 0;\n  if (!$attack(colorflip(pos), {x:square.x,y:7-square.y})) return 0;\n  return 1;\n}","links":[],"eval":true,"squares":1,"highlight":2,"forwhite":true,"graph":true,"elo":null});
...
data.push({"name":"Knight outpost","group":"Pieces","text":"<b>$</b>. Middlegame bonuses for knights outposts, bigger if outpost piece is supported by a pawn.","code":"function $$(pos, square) {\n  if (square == null) return sum(pos, $$);\n  if (board(pos, square.x, square.y) != \"N\") return 0;\n  var reachable = 0, support = 0;\n  if (!$outpost(pos, square)) {\n    reachable = $reachable_outpost(pos, square);\n    if (!reachable) return 0;\n    support = reachable > 1;\n  } else {\n    support = (board(pos, square.x - 1, square.y + 1) == \"P\"\n           || board(pos, square.x + 1, square.y + 1) == \"P\");\n  }\n  return (support ? 2 : 1) + 2 * (reachable ? 0 : 1);\n}","links":[],"eval":false,"squares":0,"highlight":0,"forwhite":true,"graph":true,"elo":{"value":"12.05","error":"4.2","link":"http://tests.stockfishchess.org/tests/view/5a774a8b0ebc5902971a9877"}});
data.push({"name":"Bishop outpost","group":"Pieces","text":"<b>$</b>. Middlegame bonuses for bishops outposts, bigger if outpost piece is supported by a pawn.","code":"function $$(pos, square) {\n  if (square == null) return sum(pos, $$);\n  if (board(pos, square.x, square.y) != \"B\") return 0;\n  var reachable = 0, support = 0;\n  if (!$outpost(pos, square)) {\n    reachable = $reachable_outpost(pos, square);\n    if (!reachable) return 0;\n    support = reachable > 1;\n  } else {\n    support = (board(pos, square.x - 1, square.y + 1) == \"P\"\n           || board(pos, square.x + 1, square.y + 1) == \"P\");\n  }\n  return (support ? 2 : 1) + 2 * (reachable ? 0 : 1);\n}","links":[],"eval":false,"squares":0,"highlight":0,"forwhite":true,"graph":true,"elo":{"value":"-0.84","error":"4.2","link":"http://tests.stockfishchess.org/tests/view/5a7749dc0ebc5902971a9875"}});
data.push({"name":"Pieces mg","group":"Pieces","text":"<b>$</b>. Middlegame bonuses and penalties to the pieces of a given color and type. Mobility not included here.","code":"function $$(pos, square) {\n  if (square == null) return sum(pos, $$);\n  if (\"NBRQ\".indexOf(board(pos, square.x, square.y)) < 0) return 0;\n  var v = 0;\n  v += [0,22,36,44,72][$knight_outpost(pos, square)];\n  v += [0,9,15,18,30][$bishop_outpost(pos, square)];\n  v += 16 * $minor_behind_pawn(pos, square);\n  v -= 3 * $bishop_pawns(pos, square);\n  v += 10 * $rook_on_pawn(pos, square);\n  v += [0,18,44][$rook_on_file(pos, square)];\n  v -= $trapped_rook(pos, square) * (98 - 22 * $mobility(pos, square)) * (pos.c[0] || pos.c[1] ? 1 : 2);\n  v -= 51 * $weak_queen(pos, square);\n  v -= 6 * $king_protector(pos, square);\n  v += 44 * $long_diagonal_bishop(pos, square);\n  return v;\n}","links":[],"eval":true,"squares":1,"highlight":2,"forwhite":true,"graph":false,"elo":null});
data.push({"name":"Pieces eg","group":"Pieces","text":"<b>$</b>. Endgame bonuses and penalties to the pieces of a given color and type. Mobility not included here.","code":"function $$(pos, square) {\n  if (square == null) return sum(pos, $$);\n  if (\"NBRQ\".indexOf(board(pos, square.x, square.y)) < 0) return 0;\n  var v = 0;\n  v += [0,6,12,12,24][$knight_outpost(pos, square)];\n  v += [0,2,5,4,10][$bishop_outpost(pos, square)];\n  v -= 8 * $bishop_pawns(pos, square);\n  v += 28 * $rook_on_pawn(pos, square);\n  v += [0,7,20][$rook_on_file(pos, square)];\n  v -= $trapped_rook(pos, square) * 5 * (pos.c[0] || pos.c[1] ? 1 : 2);\n  v -= 10 * $weak_queen(pos, square);\n  v -= 7 * $king_protector(pos, square);\n  return v;\n}","links":[],"eval":true,"squares":1,"highlight":2,"forwhite":true,"graph":false,"elo":null});
data.push({"name":"Restricted","group":"Threats","text":"<b>$</b>. Bonus for restricting their piece moves.","code":"function $$(pos, square) {\n  if (square == null) return sum(pos, $$);\n  if ($attack(pos, square) == 0) return 0;\n  var pos2 = colorflip(pos);\n  if ($attack(pos2, {x:square.x,y:7-square.y}) != 1) return 0;\n  if ($pawn_attack(pos2, {x:square.x,y:7-square.y}) > 0) return 0;\n  return 1;\n}","links":[],"eval":true,"squares":1,"highlight":2,"forwhite":true,"graph":true,"elo":null});
data.push({"name":"Threats mg","group":"Threats","text":"<b>$</b>. Middlegame threats bonus.","code":"function $$(pos) {\n  var v = 0;\n  v += 62 * $hanging(pos);\n  v += 12 * $overload(pos);\n  v += $king_threat(pos) > 0 ? 21 : 0;\n  v += 48 * $pawn_push_threat(pos);\n  v += 14 * $rank_threat(pos);\n  v += 169 * $threat_safe_pawn(pos);\n  v -= 14 * $weak_unopposed_pawn(pos);\n  v += 49 * $slider_on_queen(pos);\n  v += 20 * $knight_on_queen(pos);\n  v += 7* $restricted(pos);\n  for (var x = 0; x < 8; x++) {\n    for (var y = 0; y < 8; y++) {\n      var s = {x:x,y:y};\n      v += [0,0,39,57,68,62,0][$minor_threat(pos, s)];\n      v += [0,0,38,38,0,51,0][$rook_threat(pos, s)];\n    }\n  }\n  return v;\n}","links":[],"eval":true,"squares":0,"highlight":0,"forwhite":true,"graph":false,"elo":null});
data.push({"name":"Threats eg","group":"Threats","text":"<b>$</b>. Endgame threats bonus.","code":"function $$(pos) {\n  var v = 0;\n  v += 34 * $hanging(pos);\n  v += 6 * $overload(pos);\n  v += $king_threat(pos) > 0 ? 84 : 0;\n  v += 42 * $pawn_push_threat(pos);\n  v += 3 * $rank_threat(pos);\n  v += 99 * $threat_safe_pawn(pos);\n  v -= 20 * $weak_unopposed_pawn(pos);\n  v += 21 * $slider_on_queen(pos);\n  v += 12 * $knight_on_queen(pos);\n  v += 6 * $restricted(pos);\n  for (var x = 0; x < 8; x++) {\n    for (var y = 0; y < 8; y++) {\n      var s = {x:x,y:y};\n      v += [0,31,42,44,112,120,0][$minor_threat(pos, s)];\n      v += [0,24,71,61,38,38,0][$rook_threat(pos, s)];\n    }\n  }\n  return v;\n}","links":[],"eval":true,"squares":0,"highlight":0,"forwhite":true,"graph":false,"elo":null});
...
function command(text) {
  ...
  for (var i = 0; i < mvdivs.length; i++) {
    if (text.indexOf(mvdivs[i]) >= 0) {
      ...
      for (var j = 0; j < 2; j++) if (text2.indexOf(nmt) > 0) {
        ...
        if (j == 1 && !black) ntext = nm + ntext; else ntext += nm;
      }
  ...
  if (text.split("/").length == 8 && text.split(".").length == 1) {
    ...
    var whitename = null, blackname = null;
    var wi = text.indexOf("[White \""), bi = text.indexOf("[Black \"");
    if (wi >= 0 && bi > wi) {
      var wil = text.substr(wi+8).indexOf("\"]"), bil = text.substr(bi+8).indexOf("\"]");
    ...
    for (var i = 0; i < wb.length; i++) {
      if (wb[i].tagName != 'DIV') continue;
      var winId = wb[i].id.substring(2);
      var cur = a.find(function(x) { return x[0] == winId[0]; });
      if (cur != null && !_mobile) {
        cur = cur.substring(1);
    ...
    for (var i = 0; i < _curmoves.length; i++) if (_curmoves[i].san == text) { doMoveHandler(_curmoves[i].move); break; }
  }
}
...
function finalArrow3() {
  ...
  var list = elem.children[0].children, remElem = null;
  if (list == null) return;
...
function updateInfo() {
  ...
  // History window
  var elem = document.getElementById("history"), movesText = "";
  while (elem.firstChild) elem.removeChild(elem.firstChild);
  ...
  window.setTimeout(function() {
    ...
    for (var j = 0; j < sortArray.length; j++) {
      ...
      node1.onclick = function() {
        ...
        if (sei == null) return;
        var func = null, n2 = this.name.toLowerCase().replace(/ /g, "_");
        try { eval("func = $" + n2 + ";"); } catch (e) {}
        var elem = document.getElementById('chessboard1');
        for (var i=0; i<elem.children.length; i++) {
          var div = elem.children[i];
...
function showEvals() {
  ...
  for (var i=0; i<_curmoves.length;i++) {
    ...
    node1.onmousedown = function(e) { if (_menu) showHideMenu(false); doMoveHandler(_curmoves[this.index].move); };
...
function dragActivate() {
  ...
  _dragActive = true;
  if (!isEdit() && !_dragCtrl) showLegalMoves({x:getDragX(_startX),y:getDragY(_startY)});
...
function onMouseMove(e) {
  ...
  if (!_dragActive) {
    ...
    if (_dragLMB > 0) {
      var x1 = getDragX(_startX), y1 = getDragY(_startY), x2 = getDragX(e.clientX), y2 = getDragY(e.clientY);
      showArrow3({from:{x:x1,y:y1},to:{x:x2,y:y2}});
      _dragLMB = 2;
      return;
    }
...
function onMouseUp(e) {
  ...
  if (!_dragActive && _clickFrom != null &&  _clickFromElem != null && _clickFromElem.className.indexOf(" h0") > 0 && _dragLMB == 0) {
    ...
    _dragElement = null;
    if (!doMoveHandler({from:_clickFrom, to:{x:x2,y:y2}})) _dragElement = oldDragElement;
  }
  if (_dragElement != null) {
    var x1 = getDragX(_startX), y1 = getDragY(_startY);
    var x2 = getDragX(e.clientX), y2 = getDragY(e.clientY);  
    if (_dragActive) {
      if (!doMoveHandler({from:{x:x1,y:y1},to:{x:x2,y:y2}}, _dragCtrl ? _dragElement.className[2] : null)) {
        showBoard(true);
      } else {
...
function onKeyDown(e) {
  ...
  if (k == 96 || k == 106) { if (_engine != null && _engine.ready) command("depth " + (_engine.depth != 0 ? "0" : "10")); }
  else if (k == 107) { if (_engine != null && _engine.ready) command("depth " + Math.min(128, _engine.depth+1)); }
  else if (k == 109) { if (_engine != null && _engine.ready) command("depth " + Math.max(0, _engine.depth-1)); }
...
function parseBestMove(m) {
  ...
  var from = {x:"abcdefgh".indexOf(m[0]),y:"87654321".indexOf(m[1])};
  var to   = {x:"abcdefgh".indexOf(m[2]),y:"87654321".indexOf(m[3])};
  var p    = m.length > 4 ? "nbrq".indexOf(m[4]) : -1;
  if (p < 0) return {from:from, to:to};
  return {from:from, to:to, p: "NBRQ"[p]};
}
function evalAll() {
  ...
  if (_engine.depth == 0) { _engine.waiting = true; return; }
  var fen = getCurFEN();
  _engine.send("stop");
...
function checkSizes() {
  ...
  if (_wantUpdateInfo) { _wantUpdateInfo = false; updateInfo(); }
...
function repaintGraph(event) {
  ...
  var border1 = 4.5, border2 = 18.5;
  var xMax = 40, yMax = 2, xStep = 10, yStep = 1;
  ...
  if (event != null) {
    ...
    var b1 = border1 / _bodyScale, b2 = border2 / _bodyScale;
...
function repaintSidebars() {
  ...
  var fmat = function(mat, flip) {
    for (var i = 0; i < mat.length; i++) {
      ...
      if (flip) node1.style.top = d; else node1.style.bottom = d;
      elem.appendChild(node1);
    }
  }
  ...
  if (points != 0) {
    ...
    if (down) node1.style.bottom = d; else node1.style.top = d;
    elem.appendChild(node1);
  }
...
function reloadMenu() {
  ...
  var addMenuItemColor = function(className, text) {
    ...
    span2.onclick = function() { setBoardColor(_color+1); }

    div.appendChild(span2);
    var div1= document.createElement('div');
    div1.id = "icolor";
    div1.className = "c" + _color;
    div1.onclick = function() { setBoardColor(0); };
    var div2, div3 = document.createElement('div');
    div2= document.createElement('div'); div2.style.left = "0px"; div2.style.top = "0px"; div2.className="l"; div3.appendChild(div2);
    div2= document.createElement('div'); div2.style.left = "0px"; div2.style.top = "5px"; div2.className="d"; div3.appendChild(div2);
    div2= document.createElement('div'); div2.style.left = "5px"; div2.style.top = "0px"; div2.className="d"; div3.appendChild(div2);
    div2= document.createElement('div'); div2.style.left = "5px"; div2.style.top = "5px"; div2.className="l"; div3.appendChild(div2);
    div1.appendChild(div3);
    div.appendChild(div1);

    var span4= document.createElement('span');
    span4.id = "buttonColorPrev";
    span4.onclick = function() { setBoardColor(_color-1); }    
    div.appendChild(span4);
  ...
  addMenuItem("menuWindow", "Open in new window...", null, true, function() { command("window"); showHideMenu(false); });
}
...
function setupBoxes() {
  ...
  for (var j=0; j<elems.length; j++) for (var i=0; i<elems[j].children.length; i++) {
...
function setupDragElement(elmnt) {
  ...
  oldDisplay = elmnt.style.display; elmnt.style.display = "";
...
function setupTouchEvents(elem, funcStart, funcMove, funcEnd) {
  var onTouch = function(e) {
    ...
    switch (e.type) {
      case "touchstart": funcStart(e.changedTouches[0]); break;
      case "touchmove": funcMove(e.changedTouches[0]); break;
      case "touchend": funcEnd(e.changedTouches[0]); break;
    }
  }
...
window.onload = function() {
  ...
  document.getElementById("graphWrapper").onwheel = function(event) { onWheel(event); showGraphTooltip(_historyindex, event); };

  document.getElementById("arrowWrapper1").style.top = document.getElementById("arrowWrapper2").style.top = document.getElementById("arrowWrapper3").style.top
  = document.getElementById('chessboard1').getBoundingClientRect().top - document.getElementById("boardWrapper").getBoundingClientRect().top + "px";
  document.getElementById("arrowWrapper1").style.left = document.getElementById("arrowWrapper2").style.left = document.getElementById("arrowWrapper3").style.left
  = document.getElementById('chessboard1').getBoundingClientRect().left - document.getElementById("boardWrapper").getBoundingClientRect().left + "px";
  document.getElementById("arrowWrapper1").style.width = document.getElementById("arrowWrapper2").style.width = document.getElementById("arrowWrapper3").style.width 
  = document.getElementById("arrowWrapper1").style.height = document.getElementById("arrowWrapper2").style.height = document.getElementById("arrowWrapper3").style.height 
  = (40 * 8) + "px";
